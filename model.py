...
class PostionalEncoding(nn.Module):
    ...
    def __init__(self , d_model : init , seq_len : init , dropout : float) -> None:
            super().__init__()
            self.d_model = d_model
            self.seq_len = seq_len
            self.dropout = nn.dropout(dropout)

            # now create a matrix of shape (seq_len , 1)...why?? because its in paper
...
class EncoderBlock(nn.Module):
    ...
    def forward(self , x,src_mask):
        ...
        x = self.residual_connections[0](x,lambda x : self_attention_block(x,x,x,))
